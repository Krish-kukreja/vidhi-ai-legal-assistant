...
class CDNService:
    ...
    def __init__(
        ...
        self.enabled = enabled and os.getenv("CLOUDFRONT_ENABLED", "false").lower() == "true"
        self.region = region
        
        if self.enabled and (not self.distribution_id or not self.distribution_domain):
            logger.warning("CloudFront enabled but distribution ID or domain not configured. Falling back to S3 URLs.")
            self.enabled = False
        
        if self.enabled:
            try:
                self.cloudfront_client = boto3.client('cloudfront', region_name=region)
                logger.info(f"CDN service initialized with distribution {self.distribution_id}")
    ...
    def invalidate_cache(self, paths: list[str]) -> Dict:
        ...
        try:
            ...
            logger.info(f"CloudFront invalidation created: {invalidation_id} (status: {status})")
            ...
            logger.error(f"CloudFront invalidation failed: {error_code} - {error_message}")
    ...
    def get_invalidation_status(self, invalidation_id: str) -> Dict:
        ...
        try:
            ...
            error_code = e.response['Error']['Code']
            error_message = e.response['Error']['Message']
            logger.error(f"Failed to get invalidation status: {error_code} - {error_message}")
