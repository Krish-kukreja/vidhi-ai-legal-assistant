...
def refresh_data_pipeline(self, incremental=True):
    ...
    try:
        ...
        # Update progress
        self.update_state(
            state="PROGRESS", meta={"current": 0, "total": 100, "status": "Starting..."}
        )
...
def scrape_india_code_acts(self, resume=False, specific_acts=None):
    ...
    try:
        ...
        result = subprocess.run(
            cmd, cwd=project_root, capture_output=True, text=True, timeout=1500
        )
...
@celery_app.task(
    name="tasks.data_pipeline_tasks.cleanup_old_tasks", time_limit=300  # 5 minutes
)
