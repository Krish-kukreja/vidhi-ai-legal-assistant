    ...
    try:
        ...
        if init_message.get("type") != "start_session":
            await websocket.send_json(
                {"type": "error", "message": "First message must be start_session"}
            )
        ...
        # Sanitize inputs
        sanitized = sanitize_api_input(session_id=session_id, language=language)
        ...
        # Send confirmation
        await websocket.send_json(
            {"type": "session_started", "session_id": session_id, "language": language}
        )
        ...
        while True:
            try:
                ...
                if message_type == "audio_chunk":
                    ...
                    # Send acknowledgment
                    await websocket.send_json(
                        {"type": "chunk_received", "size": len(audio_data)}
                    )
...
async def process_voice_input(
    ...
    try:
        ...
        if not _transcribe_service:
            await websocket.send_json(
                {"type": "error", "message": "Transcription service not available"}
            )
            return
        ...
        if not _llm_service:
            await websocket.send_json(
                {"type": "error", "message": "LLM service not available"}
            )
            return
        ...
        try:
            async for event in _llm_service.query_stream(
                question=transcript, session_id=session_id, language=language
            ):
