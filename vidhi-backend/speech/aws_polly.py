...
class AWSPollyService:
    ...
    def synthesize_speech(
        self, text: str, language_code: str = "hi-IN", output_format: str = "mp3"
    ) -> Dict:
        """
        ...
        try:
            ...
            voice_config = self.voice_map.get(
                language_code, self.voice_map["hi-IN"]  # Default to Hindi
            )
            ...
            logger.info(
                f"Synthesized speech: {len(text)} chars, {len(audio_stream)} bytes"
            )
    ...
    def batch_synthesize(
        self, texts: list, bucket: str, language_code: str = "hi-IN"
    ) -> list:
        """
        ...
        for text in texts:
            result = self.synthesize_and_upload_to_s3(text, bucket, language_code)
...
class CachedPollyService:
    ...
    def __init__(
        self, polly_service: AWSPollyService, cache_bucket: str, cdn_service=None
    ):
    ...
    def get_or_create_audio(self, text: str, language_code: str = "hi-IN") -> Dict:
        """
        ...
        try:
            ...
            self.s3.put_object(
                ...
                Body=result["audio_stream"],
                ContentType="audio/mpeg",
                Metadata={"language": language_code, "text_hash": text_hash},
            )
