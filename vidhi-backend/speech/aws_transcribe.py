...
class AWSTranscribeService:
    ...
    def transcribe_audio(
        ...
        try:
            ...
            if status == "COMPLETED":
                ...
                detected_language = result["TranscriptionJob"].get(
                ...
                try:
                    ...
                    logger.error(
                        f"Error parsing transcript data: {e}. Data received: {str(transcript_data)[:200]}"
                    )
                    return {
                        "success": False,
    ...
    def _download_transcript(self, transcript_uri: str) -> Dict:
        ...
        try:
            if transcript_uri.startswith("s3://"):
                ...
                logger.info(
                    f"Downloading transcript from S3: bucket={bucket}, key={key}"
                )
    ...
    def upload_audio_to_s3(
        self, audio_data: bytes, bucket: str, key: str, content_type: str = "audio/wav"
    ) -> str:
        """
        ...
        try:
            self.s3.put_object(
                Bucket=bucket, Key=key, Body=audio_data, ContentType=content_type
            )
    ...
    def transcribe_from_file(
        self, audio_file_path: str, bucket: str, language_code: str = "hi-IN"
    ) -> Dict:
        """
...
class HybridSpeechToText:
    ...
    def transcribe(
        ...
        if not use_aws:
            ...
            key = f"transcribe-input/{int(time.time())}.wav"
            s3_uri = self.aws_transcribe.upload_audio_to_s3(audio_data, bucket, key)
