...
def search_similar_documents(
    vectorstore: Chroma, query: str, k: int = 4
) -> List[Document]:
    """
...
def create_retriever(
    vectorstore: Chroma, search_type: str = "similarity", search_kwargs: dict = None
):
    """
    ...
    try:
        ...
        if reranking_enabled:
            ...
            reranker = create_reranker(
                confidence_threshold=confidence_threshold, cache_enabled=True
            )
            ...
            class HybridRetrieverWithReranking:
                ...
                def get_relevant_documents(self, query: str) -> List[Document]:
                    # Get hybrid results
                    hybrid_results = (
                        self.hybrid_retriever.get_relevant_documents_with_scores(query)
                    )

                    # Rerank
                    reranked = self.reranker.rerank_with_metadata(
                        query, hybrid_results, top_k=self.reranking_top_k
                    )
            ...
            logger.info(
                f"Created hybrid retriever with reranking (top_k={reranking_top_k})"
            )
        else:
...
class CachedEmbeddingStore:
    ...
    def __init__(
        self, vectorstore: Chroma, cache_table_name: str = "vidhi-embedding-cache"
    ):
