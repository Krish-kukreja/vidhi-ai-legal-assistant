...
def validate_and_deduplicate(skip_validation: bool = False, skip_dedup: bool = False, auto_merge: bool = False):
    ...
    for filename, data_type in data_files:
        ...
        if not skip_dedup:
            ...
            logger.info(f"    Exact duplicates: {deduplicator.stats['exact_duplicates']}")
        ...
        logger.info(f"  Reduction: {original_count - len(data)} documents ({(original_count - len(data)) / original_count * 100:.1f}%)")
...
def main():
    ...
    parser.add_argument("--auto-merge", action="store_true", help="Automatically merge near-duplicate documents")
