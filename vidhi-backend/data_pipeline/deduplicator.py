...
class Deduplicator:
    ...
    def find_near_duplicates(self, doc: Dict[str, Any], content_hash: str) -> List[Tuple[Dict, float]]:
        """
        ...
        for existing_hash, existing_doc in self.hash_map.items():
            ...
            existing_content = self.normalize_content(self.extract_content(existing_doc))
    ...
    def merge_documents(self, doc1: Dict[str, Any], doc2: Dict[str, Any]) -> Dict[str, Any]:
        ...
        merged['_merged_from'] = [
            doc1.get('scheme_name') or doc1.get('article_no') or doc1.get('act_name') or 'unknown',
            doc2.get('scheme_name') or doc2.get('article_no') or doc2.get('act_name') or 'unknown'
        ]
        
        return merged
    
    def process_document(self, doc: Dict[str, Any], auto_merge: bool = False) -> Tuple[bool, Optional[Dict]]:
        """
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "near_duplicates_sample": [
                {
                    "doc1": doc1.get('scheme_name') or doc1.get('article_no') or 'unknown',
                    "doc2": doc2.get('scheme_name') or doc2.get('article_no') or 'unknown',
    ...
    def save_report(self, output_dir: str = None):
        ...
        report_path = os.path.join(
            output_dir,
            f"dedup_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python deduplicator.py <input_file> [output_file] [--auto-merge] [--threshold=0.85]")
        sys.exit(1)
