...
class CheckpointManager:
    ...
    def save(self):
        ...
        try:
            ...
            logger.info(
                f"Checkpoint saved: {len(self.state['completed_acts'])} acts completed"
            )
        except Exception as e:
...
def get_act_sections(year_id: str) -> Tuple[List[Dict], Optional[str]]:
    ...
    try:
        ...
        section_links = [
            ...
            or "section" in a.get("href", "").lower()
        ]
        ...
        if result_links:
            ...
            def fetch_act():
                act_resp = requests.get(
                    act_url, timeout=30, headers={"User-Agent": "Mozilla/5.0"}
                )
            ...
            if content:
                ...
                for i, start in enumerate(range(0, len(text), 2000)):
                    sections.append(
                        {"chunk_index": i, "text": text[start : start + 2000]}
                    )
...
def fetch_all_acts(resume: bool = False, specific_acts: Optional[List[str]] = None):
    ...
    if specific_acts:
        acts_to_scrape = [
            (name, id) for name, id in PRIORITY_ACTS if id in specific_acts
        ]
    ...
    if resume:
        ...
        logger.info(
            f"Already completed: {len(checkpoint.state['completed_acts'])} acts"
        )
    ...
    for idx, (act_name, act_id) in enumerate(
        acts_to_scrape[start_index:], start=start_index
    ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Scrape Indian legal acts from indiacode.nic.in"
    )
    parser.add_argument(
        "--resume", action="store_true", help="Resume from last checkpoint"
    )
    parser.add_argument(
        "--acts", nargs="+", help="Specific act IDs to scrape (e.g., 1860/45 1974/2)"
    )
    parser.add_argument(
        "--clear-checkpoint",
        action="store_true",
