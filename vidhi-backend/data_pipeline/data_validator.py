...
class GovernmentScheme(BaseModel):
    ...
    def completeness_score(self) -> float:
        ...
        valid_fields = sum(
            1 for f in fields
            if f and f.strip() and f.strip().lower() not in ['not available', 'n/a', 'na', '']
        )
...
class DataValidator:
    ...
    def get_needs_review(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            for r in self.results
            if r.status == ValidationStatus.WARNING and r.completeness_score and r.completeness_score < 70
        ]
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": self.stats,
            "pass_rate": (self.stats["valid"] / self.stats["total"] * 100) if self.stats["total"] > 0 else 0,
            "by_type": {}
        }
        ...
        for data_type in DataType:
            ...
            if type_results:
                report["by_type"][data_type.value] = {
                    ...
                    "valid": sum(1 for r in type_results if r.status == ValidationStatus.VALID),
                    "warning": sum(1 for r in type_results if r.status == ValidationStatus.WARNING),
                    "invalid": sum(1 for r in type_results if r.status == ValidationStatus.INVALID),
                }
    ...
    def save_review_queue(self):
        ...
        if needs_review:
            ...
            logger.info(f"Review queue saved: {review_path} ({len(needs_review)} items)")
