...
class VidhiAgent:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {"use_tools": "execute_tools", "skip_tools": "generate"},
        )
        ...
        workflow.add_conditional_edges(
            ...
            {"continue": "select_tools", "finish": "generate"},
        )
...
Return ONLY the alternative queries, one per line, without numbering or explanation."""
            ...
            state["reasoning_steps"] = [
                f"Expanded query into {len(expansions)} variations"
            ]
...
Respond with ONLY the tool names that should be used, one per line. If no tools are needed, respond with "NONE"."""
            ...
            for tool in selected_tools:
                state["tool_calls"].append(
                    {"tool": tool, "status": "pending", "result": None}
                )

            state["reasoning_steps"].append(
                f"Selected {len(selected_tools)} tools: {selected_tools}"
            )
                ...
                if tool_call["status"] == "pending":
                    ...
                    if "search_legal_database" in tool_name:
                        ...
                        if section and act:
                            ...
                            result = {
                                "error": "Could not extract section and act from query"
                            }
                        ...
                        result = {
                            "error": "Limitation period calculation requires offense type and date"
                        }
    ...
    def _reason_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if has_results:
                ...
                state["reasoning_steps"].append(
                    "Insufficient information, may need more tools"
                )
...
Format using Markdown."""
    ...
    def _confidence_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if retrieved_docs:
                doc_confidences = [
                    doc.get("confidence", 0.5) for doc in retrieved_docs[:5]
                ]
    ...
    def query(self, query: str, language: str = "English") -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Agent completed: confidence={response['confidence']}, iterations={response['iterations']}"
            )
            return response
