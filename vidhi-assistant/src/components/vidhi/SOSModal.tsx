    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          <motion.div
            className="relative z-10 w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl bg-card border-2 border-sos shadow-2xl"
            initial={{ scale: 0.9, y: 20 }}
            animate={{ scale: 1, y: 0 }}
            exit={{ scale: 0.9, y: 20 }}
          >
            <div className="sticky top-0 bg-sos px-6 py-4 rounded-t-2xl flex items-center justify-between">
              ...
              {rights.map((right, i) => (
                ...
                >
                  <h3 className="font-bold text-foreground text-lg flex items-center gap-2">
                    <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-sos text-sos-foreground text-sm font-bold">
                      {i + 1}
                    </span>
                    {right.title}
                  </h3>
